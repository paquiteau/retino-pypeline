...
def retino_preprocess(working_dir, output_dir):
    ...
    preproc_input_node = Node(util.IdentityInterface(fields=["AP", "PA", "T1w"]), name="inputspec")
    # 1. Realign  with SPM
    ...
    topup.inputs.encoding_file = os.path.join(os.path.abspath(os.getcwd()), "acq_params.txt")
    ...
    applytopup.inputs.encoding_file = os.path.join(os.path.abspath(os.getcwd()), "acq_params.txt")
    ...
    output_node = Node(util.IdentityInterface(fields=["preprocessed", "motion_par"]), name="outputspec")
    # Connect Workflow
    ...
    retino_input_node = Node(nio.SelectFiles(templates,
    ...
    sinker.inputs.base_directory = os.path.realpath(os.path.join(output_dir, os.path.pardir))
    sinker.inputs.substitutions = [("rrEPI", "EPI"),
                                ("rp_EPI", "EPI"),
                                ]
    ...
    retino_preprocess.connect(retino_input_node, "clockwise", preproc_clock, "inputspec.AP")
    retino_preprocess.connect(retino_input_node, "t1w", preproc_clock, "inputspec.T1w")

    retino_preprocess.connect(retino_input_node, "anticlockwise", preproc_anticlock, "inputspec.AP")
    retino_preprocess.connect(retino_input_node, "t1w", preproc_anticlock, "inputspec.T1w")
    ...
    retino_preprocess.connect(preproc_clock, "outputspec.preprocessed", sinker, "preproc.@clock_preprocessed")
    retino_preprocess.connect(preproc_anticlock, "outputspec.preprocessed", sinker, "preproc.@anticlock_preprocessed")

    retino_preprocess.connect(preproc_anticlock, "outputspec.motion_par", sinker, "preproc.@anticlock_motion_par")
    retino_preprocess.connect(preproc_clock, "outputspec.motion_par", sinker, "preproc.@clock_motion_clock")
